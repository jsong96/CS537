/***********************************************************************
* FILENAME :       main_fifo.c             
*
* DESCRIPTION :
*       This is a main source file for fifo page replacement algorithm
*
*
 ...
 *     unique pids. It rereads the file and simulate fifo page replacement algorithm
...
int main(int argc, char **argv)
{
  ...
  if (f->filename != NULL)
  {
    ...
    pid_list = NULL;
    printf("\n------------------ RUNNING %s --------------------------\n", f->filename);
    dq = fifo(f->filename, pagetable_size, uniquelist);
    printf("\n------------------ Statistic  -----------------------\n");
    printf("AMU: %f\n", dq->amu);
    printf("ARP: %f\n", dq->arp);
    printf("TMR: %d\n", dq->tmr);
